...
function Tag({ children, tone = 'neutral' }: { children: ReactNode; tone?: 'neutral' | 'purple' | 'signal' | 'success' | 'warning' }) {
  return <span className={`tag tag--${tone}`}>{children}</span>
}

function Panel({ children, className = '', title, index, action }: { children: ReactNode; className?: string; title?: string; index?: string; action?: ReactNode }) {
  return (
...
function ConfirmDialog({ title, text, confirmLabel, busy = false, onCancel, onConfirm }: { title: string; text: string; confirmLabel: string; busy?: boolean; onCancel: () => void; onConfirm: () => void }) {
...
function Metric({ label, value, unit, delta, signal = false }: { label: string; value: string; unit?: string; delta?: string; signal?: boolean }) {
  return (
...
function Bars({ values, accentAt = -1 }: { values: number[]; accentAt?: number }) {
  return (
...
function ScreenHeader({ eyebrow, title, text, action }: { eyebrow: string; title: string; text: string; action?: ReactNode }) {
  return (
