...
export function MessageThread({ profile, coachId, athleteId, counterpartName }: { profile: AppProfile; coachId: string; athleteId: string; counterpartName: string }) {
  const [messages, setMessages] = useState<Message[]>([]); const [body, setBody] = useState(''); const [error, setError] = useState(''); const [sending, setSending] = useState(false); const listRef = useRef<HTMLDivElement | null>(null)
  useEffect(() => {
    let active = true
    ...
    const unsubscribe = subscribeToMessages(profile, message => { if (!active) return; setMessages(current => current.some(item => item.id === message.id) ? current : [...current, message]); if (message.senderUserId !== profile.userId) void markConversationRead(profile, coachId, athleteId).catch(reason => { if (active) setError(reason instanceof Error ? reason.message : 'Stato di lettura non aggiornato.') }) }, { coachId, athleteId })
